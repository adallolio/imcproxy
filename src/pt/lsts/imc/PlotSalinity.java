...
public class PlotSalinity {
	...
	static Vector<String> salinity = new Vector<String>(); 
	...
	// String for influxdb.
	static String influxdb = "--input /home/autonaut/java_to_influx/salinity.csv --user autonaut --password ntnu_autonaut --dbname AUTONAUT --metricname salinity --fieldcolumns value";
    ...
    static void plot(IMCMessage message){
		...
		System.out.println("Salinity record saved!");
		...
		if(salinity.size() == max_size_1000)
		{
		...
		System.out.println(salinity.size() + " " + date_csv);
		...
		if(plot)
		{
			...
			System.out.println(salinity.size() + " " + times.size());

			try (PrintWriter writer = new PrintWriter(new File("/home/autonaut/java_to_influx/salinity.csv"))) {
				...
				for(int i=0; i<salinity.size(); i++)
				{
					sb.append(times.get(i));
					sb.append(',');
					sb.append(salinity.get(i));
					sb.append('\n');
				}
			...
			System.out.println(salinity.size() + " " + times.size());
			// Create Chart
			XYChart chart = new XYChartBuilder().width(600).height(500).title("Salinity - (last update "+date_title+" at "+date_x_axis+ ")").xAxisTitle("Time").yAxisTitle("V").build();
			...
			chart.addSeries("salinity", times, salinity);
			
			// Save it
			try {
				BitmapEncoder.saveBitmap(chart, "/var/www/dokuwiki/data/media/salinity-rt", BitmapFormat.PNG);
			} catch(IOException e) {
			}
			prev_date_plot = curr_date;*/
		}
