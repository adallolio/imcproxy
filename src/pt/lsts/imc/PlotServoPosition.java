...
public class PlotServoPosition {
	...
	static Vector<String> servos = new Vector<String>(); 
	...
	// String for influxdb.
	static String influxdb = "--input /home/autonaut/java_to_influx/servo.csv --user autonaut --password ntnu_autonaut --dbname AUTONAUT --metricname servo --fieldcolumns value";
    ...
    static void plot(IMCMessage message){
        ...
        Double temp = message.getDouble("value");
        Double temp_double = temp*(180.0/Math.PI);
        String servo = Double.toString(temp_double);
        servos.add(servo);
		times.add(date_csv);

		System.out.println(servos.size() + " " + date_csv);
		...
		if(plot)
		{
			...
			System.out.println(servos.size() + " " + times.size());

			try (PrintWriter writer = new PrintWriter(new File("/home/autonaut/java_to_influx/servo.csv"))) {
