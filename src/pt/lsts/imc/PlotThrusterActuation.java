...
public class PlotThrusterActuation {
	...
	static Vector<String> actuations = new Vector<String>(); 
	...
	// String for influxdb.
	static String influxdb = "--input /home/autonaut/java_to_influx/thruster.csv --user autonaut --password ntnu_autonaut --dbname AUTONAUT --metricname thruster --fieldcolumns value";
    ...
    static void plot(IMCMessage message){
		...
		if(actuations.size() == max_size_1000)
		{
        ...
        actuations.add(temp_string);
		times.add(date_csv);

		System.out.println(actuations.size() + " " + date_csv);
		...
		if(plot)
		{
			...
			System.out.println(actuations.size() + " " + times.size());

			try (PrintWriter writer = new PrintWriter(new File("/home/autonaut/java_to_influx/thruster.csv"))) {
