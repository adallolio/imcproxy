...
public class PlotConductivity {
	...
	static Vector<String> conductivity = new Vector<String>(); 
	...
	// String for influxdb.
	static String influxdb = "--input /home/autonaut/java_to_influx/conductivity.csv --user autonaut --password ntnu_autonaut --dbname AUTONAUT --metricname conductivity --fieldcolumns value";
    ...
    static void plot(IMCMessage message){
		...
		if(conductivity.size() == max_size_1000)
		{
		...
		for (Map.Entry<String, Object> entry : values.entrySet()) {
			...
			conductivity.add(value);
		}

		times.add(date_csv);

		System.out.println(conductivity.size() + " " + date_csv);
		...
		if(plot)
		{
			...
			try (PrintWriter writer = new PrintWriter(new File("/home/autonaut/java_to_influx/conductivity.csv"))) {
