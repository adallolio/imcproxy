...
public class PlotEstimatedFreq {
	...
	static Vector<String> freq = new Vector<String>(); 
	...
	// String for influxdb.
	static String influxdb = "--input /home/autonaut/java_to_influx/wavefreq.csv --user autonaut --password ntnu_autonaut --dbname AUTONAUT --metricname wavefreq --fieldcolumns value";
    ...
    static void plot(IMCMessage message){
		...
		System.out.println("EstimatedFreq record saved!");
		...
		System.out.println(freq.size() + " " + date_csv);
		...
		if(plot)
		{
			...
			try (PrintWriter writer = new PrintWriter(new File("/home/autonaut/java_to_influx/wavefreq.csv"))) {
			...
			chart.addSeries("freq", times, freq);
