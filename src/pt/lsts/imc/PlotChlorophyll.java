...
public class PlotChlorophyll {
	...
	static Vector<String> chlorophyll = new Vector<String>(); 
	...
	// String for influxdb.
	static String influxdb = "--input /home/autonaut/java_to_influx/chlorophyll.csv --user autonaut --password ntnu_autonaut --dbname AUTONAUT --metricname chlorophyll --fieldcolumns value";
    ...
    static void plot(IMCMessage message){
		...
		System.out.println("Chlorophyll record saved!");
		...
		if(chlorophyll.size() == max_size_1000)
		{
		...
		for (Map.Entry<String, Object> entry : values.entrySet()) {
			...
			chlorophyll.add(value);
		}

		times.add(date_csv);

		System.out.println(chlorophyll.size() + " " + date_csv);
		...
		if(plot)
		{
			...
			try (PrintWriter writer = new PrintWriter(new File("/home/autonaut/java_to_influx/chlorophyll.csv"))) {
				...
				for(int i=0; i<chlorophyll.size(); i++)
				{
					sb.append(times.get(i));
					sb.append(',');
					sb.append(chlorophyll.get(i));
					sb.append('\n');
				}
			...
			chart.addSeries("chlorophyll", times, chlorophyll);
			
			// Save it
			try {
				BitmapEncoder.saveBitmap(chart, "/var/www/dokuwiki/data/media/chlorophyll-rt", BitmapFormat.PNG);
			} catch(IOException e) {
			}
			prev_date_plot = curr_date;
			*/
