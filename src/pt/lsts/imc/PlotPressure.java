...
public class PlotPressure {
	...
	static Vector<String> pressure = new Vector<String>(); 
	...
	// String for influxdb.
	static String influxdb = "--input /home/autonaut/java_to_influx/pressure.csv --user autonaut --password ntnu_autonaut --dbname AUTONAUT --metricname pressure --fieldcolumns value";
    ...
    static void plot(IMCMessage message){
		...
		System.out.println("Pressure record saved!");
		...
		if(pressure.size() == max_size_1000)
		{
		...
		System.out.println(pressure.size() + " " + date_csv);
		...
		if(plot)
		{
			...
			try (PrintWriter writer = new PrintWriter(new File("/home/autonaut/java_to_influx/pressure.csv"))) {
			...
			chart.addSeries("pressure", times, pressure);
			
			// Save it
			try {
				BitmapEncoder.saveBitmap(chart, "/var/www/dokuwiki/data/media/pressure-rt", BitmapFormat.PNG);
			} catch(IOException e) {
			}
			prev_date_plot = curr_date;*/
		}
