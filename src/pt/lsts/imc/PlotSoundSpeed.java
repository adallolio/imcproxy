...
public class PlotSoundSpeed {
	...
	static Vector<String> sspeed = new Vector<String>(); 
	...
	// String for influxdb.
	static String influxdb = "--input /home/autonaut/java_to_influx/soundspeed.csv --user autonaut --password ntnu_autonaut --dbname AUTONAUT --metricname soundspeed --fieldcolumns value";
    ...
    static void plot(IMCMessage message){
		...
		System.out.println("SoundSpeed record saved!");
		...
		System.out.println(sspeed.size() + " " + date_csv);
		...
		if(plot)
		{
			...
			try (PrintWriter writer = new PrintWriter(new File("/home/autonaut/java_to_influx/soundspeed.csv"))) {
				...
				for(int i=0; i<sspeed.size(); i++)
				{
					sb.append(times.get(i));
					sb.append(',');
					sb.append(sspeed.get(i));
					sb.append('\n');
				}
			...
			chart.addSeries("sspeed", times, sspeed);
			
			// Save it
			try {
				BitmapEncoder.saveBitmap(chart, "/var/www/dokuwiki/data/media/soundspeed-rt", BitmapFormat.PNG);
			} catch(IOException e) {
			}
			prev_date_plot = curr_date;*/
		}
